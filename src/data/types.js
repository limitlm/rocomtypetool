...
export function getMultiDefenseCoverage(typeIds) {
  ...
  for (const attackerType of Object.keys(TYPE_CHART)) {
    ...
    for (const defenderId of typeIds) {
      const multiplier = getMultiplier(attackerType, defenderId)
      if (multiplier < minMultiplier) {
        minMultiplier = multiplier
      }
    }
    coverage[attackerType] = minMultiplier
  }
...
export function analyzeMultiTeam(typeIds) {
  ...
  for (const [attackerType, multiplier] of Object.entries(defenseCoverage)) {
    if (multiplier >= 2) {
      ...
      strongResistAgainst.push(attackerType)
    }
  }

  const attackScore = Math.round((superEffectiveTargets.length / Object.keys(TYPE_CHART).length) * 100)
  const defenseScore = Math.min(Math.round(((resistAgainst.length + strongResistAgainst.length * 1.5) / Object.keys(TYPE_CHART).length) * 100), 100)
  ...
  return {
    ...
    defenseCoverage: {
      ...
      strongResist: strongResistAgainst,
      score: defenseScore
    },
    typeIds
  }
